...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        protected_emails = options.get("emails") or getattr(
            settings, "PROTECTED_SUPERUSER_EMAILS", []
        )

        if not protected_emails:
            self.stdout.write(
                self.style.ERROR(
                    "No protected emails defined in settings.PROTECTED_SUPERUSER_EMAILS"
                )
            )
            return
        ...
        for user in superusers:
            if user.email not in protected_emails:
                ...
                if options["fix"]:
                    ...
                    user.save()
                    self.stdout.write(
                        self.style.WARNING("   ↳ Fixed: removed superuser privileges")
                    )
            else:
                self.stdout.write(self.style.SUCCESS(f"✅ Authorized: {user.email}"))
        ...
        if unauthorized_count > 0:
            ...
            if not options["fix"]:
                self.stdout.write(
                    self.style.WARNING(
                        "Run with --fix to automatically deactivate them"
                    )
                )
        else:
            self.stdout.write(self.style.SUCCESS("✅ All superusers are authorized!"))
