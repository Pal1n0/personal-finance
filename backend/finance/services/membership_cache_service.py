...
from ..models import WorkspaceAdmin, WorkspaceMembership
...
class MembershipCacheService:
    ...
    def get_user_workspace_role(self, user_id, workspace_id):
        ...
        if comprehensive_data:
            role = comprehensive_data["roles"].get(workspace_id)
            if role:
                logger.debug(
    ...
    def is_user_workspace_member(self, user_id, workspace_id):
        ...
        comprehensive_data = cache.get(f"comprehensive_membership_{user_id}")
        if comprehensive_data:
            is_member = workspace_id in comprehensive_data["roles"]
            logger.debug(
    ...
    def _fetch_optimized_user_data(self, user_id):
        ...
        memberships = (
            ...
            .values("workspace_id", "role", "workspace__name", "workspace__is_active")
        )

        admin_workspaces = set(
            WorkspaceAdmin.objects.filter(user_id=user_id, is_active=True).values_list(
        ...
        logger.debug(
            ...
            extra={
                "user_id": user_id,
                "memberships_count": len(comprehensive_data["roles"]),
