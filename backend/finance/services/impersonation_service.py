...
class ImpersonationService:
    ...
    def validate_impersonation_target(self, admin_user, target_user):
        ...
        if (
            ...
            and target_user.email not in self.ALLOWED_SUPERUSER_EMAILS
        ):
            logger.critical(
    ...
    def process_superuser_impersonation(self, admin_user, target_user, workspace_id):
        ...
        """
        if workspace_id:
            is_member = self.membership_service.is_user_workspace_member(
                target_user.id, workspace_id
            )
            if is_member:
                logger.info(
    ...
    def process_workspace_admin_impersonation(
        ...
        if workspace_id:
            ...
            admin_data = self.membership_service.get_comprehensive_user_data(
                admin_user.id
            )
            target_data = self.membership_service.get_comprehensive_user_data(
                target_user.id
            )
    ...
    def process_impersonation(self, admin_user, target_user_id, workspace_id):
        ...
        if admin_user.is_superuser:
            workspace_ids = self.process_superuser_impersonation(
                admin_user, target_user, workspace_id
            )
            return target_user, True, "superuser", workspace_ids
        else:
            workspace_ids = self.process_workspace_admin_impersonation(
                admin_user, target_user, workspace_id
            )
            if workspace_ids:
