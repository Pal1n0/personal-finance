...
class ExchangeRateFactory(DjangoModelFactory):
    ...
    rate_to_eur = factory.LazyAttribute(
        lambda _: fake.pydecimal(left_digits=1, right_digits=6, min_value=0.5, max_value=1.5)
    )
...
class TransactionDraftFactory(DjangoModelFactory):
    ...
    transactions_data = factory.LazyFunction(
        lambda: [
            {
                'type': 'expense',
                'original_amount': str(fake.pydecimal(left_digits=3, right_digits=2, min_value=1, max_value=500)),
...
class LargeTransactionFactory(TransactionFactory):
    original_amount = factory.LazyAttribute(
        lambda _: fake.pydecimal(left_digits=6, right_digits=2, min_value=100000, max_value=1000000)
    )
        ...
        for level in range(1, levels + 1):
            ...
            for i in range(categories_per_level):
                category = ExpenseCategoryFactory(
                    version=version,
                    name=f"Level {level} Expense {i+1}",
                    level=level
                )
...
class BulkDataFactory:
    ...
    def create_bulk_transactions(cls, user, workspace, count=50):
        ...
        expense_categories = ExpenseCategoryFactory.create_batch(5, version=expense_version)
        income_categories = IncomeCategoryFactory.create_batch(5, version=income_version)
        ...
        for i in range(count):
            ...
            transaction = TransactionFactory(
                ...
                expense_category=fake.random_element(expense_categories) if is_expense else None,
                income_category=fake.random_element(income_categories) if not is_expense else None,
