...
from finance.mixins import WorkspacePermissionMixin
from finance.models import Workspace, WorkspaceMembership, WorkspaceAdmin

User = get_user_model()


class TestViewSet(WorkspacePermissionMixin, APIView):
    """
    Testovací ViewSet pre mixin - simuluje reálny ViewSet
    """
    pass


class TestWorkspacePermissionMixin(TestCase):
    """
    Comprehensive test suite for WorkspacePermissionMixin.
    """
    
    def setUp(self):
        """Set up test data with users, workspaces, and admin assignments."""
        self.factory = RequestFactory()
        self.viewset = TestViewSet()  # ← Použijeme testovací ViewSet
    ...
    def create_request(self, user=None, method='GET', data=None, kwargs=None):
        ...
        # Nastavíme kwargs pre mixin
        self.viewset.kwargs = kwargs or {}
        return request

    def initialize_mixin(self, request):
        """Helper to properly initialize the mixin with request."""
        # Inicializujeme request atribúty pred volaním initial
        self.viewset._initialize_request_defaults(request)
        # Voláme initial metódu
        self.viewset.initial(request)

    def test_initialization_sets_defaults(self):
        """Test that request defaults are properly initialized for security."""
        request = self.create_request()
        
        # Volanie initial metódy mixinu
        self.initialize_mixin(request)
    ...
    def test_impersonation_rate_limiting(self, mock_cache):
        ...
        result = self.viewset._check_impersonation_rate_limit(request)
    ...
    def test_user_id_parameter_extraction(self):
        ...
        user_id = self.viewset._get_user_id_param(request)
    ...
    def test_workspace_id_extraction_and_validation(self):
        ...
        request = self.create_request(kwargs={'workspace_pk': str(self.workspace.id)})
        
        workspace_id = self.viewset._get_validated_workspace_id(request)
    ...
    def test_nonexistent_workspace_id_validation(self):
        ...
        request = self.create_request(kwargs={'workspace_pk': '9999'})
    ...
    def test_invalid_workspace_id_format(self):
        ...
        request = self.create_request(kwargs={'workspace_pk': 'invalid'})
    ...
    def test_basic_permissions_set_for_superuser(self):
        ...
        # Inicializujeme request pred testom
        self.viewset._initialize_request_defaults(request)
        self.viewset._set_basic_permissions(request)
    ...
    def test_self_impersonation_validation_blocked(self):
        ...
        result = self.viewset._validate_impersonation_target(self.regular_user, self.regular_user)
        
        self.assertFalse(result)

    def test_superuser_impersonation_by_non_superuser_blocked(self):
        """Test that non-superusers cannot impersonate superusers."""
        result = self.viewset._validate_impersonation_target(self.regular_user, self.superuser)
        
        self.assertFalse(result)

    def test_valid_impersonation_target(self):
        """Test that valid impersonation targets are approved."""
        result = self.viewset._validate_impersonation_target(self.admin_user, self.regular_user)
        
        self.assertTrue(result)

    @patch('finance.mixins.get_user_model')
    def test_superuser_impersonation_processing_single_workspace(self, mock_user_model):
        ...
        request = self.create_request(self.superuser, kwargs={'workspace_pk': str(self.workspace.id)})
        
        # Inicializujeme request
        self.viewset._initialize_request_defaults(request)
        request.user_permissions['is_superuser'] = True
        request.user_permissions['workspace_exists'] = True
        
        with patch.object(self.viewset, '_is_user_workspace_member', return_value=True):
            self.viewset._process_impersonation_request(request, self.target_user.id, self.workspace.id)
    ...
    def test_superuser_impersonation_processing_all_workspaces(self, mock_user_model):
        ...
        with patch.object(self.viewset, '_get_user_workspace_ids', return_value=user_workspaces):
            self.viewset._process_impersonation_request(request, self.target_user.id, None)
    ...
    def test_workspace_admin_impersonation_processing_single_workspace(self, mock_user_model):
        ...
        mock_user_model.return_value.objects.get.return_value = self.target_user
        request = self.create_request(self.admin_user, kwargs={'workspace_pk': str(self.workspace.id)})
        ...
        with patch.object(self.viewset, '_can_admin_impersonate_in_workspace', return_value=True):
            self.viewset._process_impersonation_request(request, self.target_user.id, self.workspace.id)
    ...
    def test_workspace_admin_impersonation_processing_multiple_workspaces(self, mock_user_model):
        ...
        with patch.object(self.viewset, '_get_common_admin_workspaces', return_value=common_workspaces):
            self.viewset._process_impersonation_request(request, self.target_user.id, None)
    ...
    def test_workspace_admin_impersonation_processing_permission_denied(self, mock_user_model):
        ...
        with patch.object(self.viewset, '_can_admin_impersonate_in_workspace', return_value=False):
            self.viewset._process_impersonation_request(request, self.target_user.id, self.workspace.id)
    ...
    def test_impersonation_user_not_found(self, mock_user_model):
        ...
        with patch.object(User.objects, 'get', side_effect=User.DoesNotExist):
            self.viewset._process_impersonation_request(request, 9999, self.workspace.id)
        
        self.assertFalse(request.is_admin_impersonation)

    def test_workspace_access_processing_for_member(self):
        """Test workspace access processing for valid workspace members."""
        request = self.create_request(self.regular_user, kwargs={'workspace_pk': str(self.workspace.id)})
        
        # Inicializujeme request
        self.viewset._initialize_request_defaults(request)
        request.user_permissions['workspace_exists'] = True
        
        with patch.object(self.viewset, '_get_user_workspace_role', return_value='viewer'):
            self.viewset._process_workspace_access(request, self.workspace.id)
    ...
    def test_workspace_access_processing_for_non_member(self):
        ...
        with patch.object(self.viewset, '_get_user_workspace_role', return_value=None):
            self.viewset._process_workspace_access(request, self.workspace.id)
    ...
    def test_workspace_access_to_nonexistent_workspace(self):
        ...
        request = self.create_request(self.regular_user, kwargs={'workspace_pk': '9999'})
        
        # Inicializujeme request
        self.viewset._initialize_request_defaults(request)
        request.user_permissions['workspace_exists'] = False
        
        self.viewset._process_workspace_access(request, 9999)
    ...
    def test_workspace_admin_check_with_caching(self):
        ...
        with patch.object(cache, 'get', return_value=True):
            result = self.viewset._is_workspace_admin(self.admin_user, self.workspace.id)
    ...
    def test_workspace_membership_check_with_caching(self):
        ...
        with patch.object(cache, 'get', return_value=True):
            result = self.viewset._is_user_workspace_member(self.regular_user, self.workspace.id)
    ...
    def test_user_workspace_role_retrieval(self):
        ...
        with patch.object(cache, 'get', return_value='viewer'):
            role = self.viewset._get_user_workspace_role(self.regular_user, self.workspace.id)
    ...
    def test_user_workspace_ids_retrieval(self):
        ...
        with patch.object(cache, 'get', return_value=[self.workspace.id]):
            workspace_ids = self.viewset._get_user_workspace_ids(self.regular_user)
    ...
    def test_common_admin_workspaces_calculation(self):
        ...
        with patch.object(self.viewset, '_get_user_workspace_ids', side_effect=[admin_workspaces, target_workspaces]):
            common = self.viewset._get_common_admin_workspaces(self.admin_user, self.target_user)
    ...
    def test_single_workspace_impersonation_grant(self):
        ...
        self.viewset._grant_workspace_impersonation(request, self.target_user, self.workspace.id)
    ...
    def test_multiple_workspaces_impersonation_grant(self):
        ...
        self.viewset._grant_multiple_workspaces_impersonation(request, self.target_user, workspace_ids)
    ...
    def test_impersonation_reset(self):
        ...
        self.viewset._reset_impersonation(request)
    ...
    def test_database_error_handling(self, mock_logger):
        ...
        with patch.object(self.viewset, '_initialize_request_defaults') as mock_init:
            mock_init.side_effect = DatabaseError("DB connection failed")
            
            # Simulácia volania cez process_workspace_permissions
            self.viewset._process_workspace_permissions(request)
    ...
    def test_complete_impersonation_flow(self):
        ...
        request = self.create_request(
            self.superuser, 
            data={'user_id': str(self.target_user.id)},
            kwargs={'workspace_pk': str(self.workspace.id)}
        )
        
        with patch.object(self.viewset, '_check_impersonation_rate_limit', return_value=True):
            with patch.object(self.viewset, '_validate_impersonation_target', return_value=True):
                with patch.object(self.viewset, '_is_user_workspace_member', return_value=True):
                    # Volanie initial metódy mixinu
                    self.initialize_mixin(request)
    ...
    def test_unauthorized_superuser_email_blocked(self):
        ...
        result = self.viewset._validate_impersonation_target(
            self.superuser, 
    ...
    def test_unauthorized_superuser_email_blocked_duplicate(self):
        ...
        result = self.viewset._validate_impersonation_target(self.superuser, unauthorized_superuser)
        self.assertFalse(result)